...
"""The single policy decision core shared by both enforcement surfaces.

Both surfaces of the control route every governed action through :func:`decide`:

- the ``policy_auth`` Auth plugin clamps token scopes at issuance, and
- the :class:`~nest_plugins_reference.policy.enforcer.PolicyEnforcer` proxy
  blocks out-of-policy calls at runtime (raising :class:`PolicyViolationError`).

Keeping one decision function means the registered plugin and the proxy can
never disagree on what the policy permits.

:func:`decide` is a *pure* read over the manifest and the current
:class:`PolicyState` — it never mutates state and never raises. The caller
records spend / grants approvals on a successful action, then asks again.
...
class PolicyViolationError(Exception):
    """Raised by the enforcement proxy when an action exceeds the manifest.

    Carries the offending ``agent_id``, ``op``, and a human-readable ``reason``.
    Defined alongside the decision core so both enforcement surfaces share one
    error type.

    Example::

        raise PolicyViolationError(AgentId("a1"), "pay", "budget exceeded")
    """

    def __init__(self, agent_id: AgentId, op: str, reason: str) -> None:
