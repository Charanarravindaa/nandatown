...
cap). Cumulative spend and approval gating are stateful and enforced at runtime
by the :class:`~nest_plugins_reference.policy.enforcer.PolicyEnforcer`, not by a
durable, stateless token scope. ``register`` (capability declaration) is a
runtime action gated by the enforcer and has no token-scope form by design.
